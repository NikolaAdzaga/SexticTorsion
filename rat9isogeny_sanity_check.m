...
//
// IMPORTANT:
// We keep the generators exactly as listed in the source,
// and implement the action explicitly on the LEFT.
...
// Does J fix a primitive order-9 vector?
...
DegreeOfVector := function(Hsub, v)
    return Index(Hsub, FixerSubgroup(Hsub, v));
end function;
...
HasStableCyclicOrd9Subgroup := function(Hsub)
    for v in V9 do
        ...
        for g in Generators(Hsub) do
            if not SameCyclicSubgroup(v, Act(g, v)) then
                stable := false;
                break;
            end if;
        end for;
