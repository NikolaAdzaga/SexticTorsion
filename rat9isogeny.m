// This is for the second case in Proposition 3.4.
...
// The two mod-3 groups from Table 3
B311 := sub< G3 |
    G3![1,0,0,2],
    G3![1,1,0,1]
>;
...
HasTargetMod3Image := function(Hbar)
    for g in G3 do
        if Hbar^g eq B311 then
            return true, "3B.1.1";
        end if;
        if Hbar^g eq B312 then
            return true, "3B.1.2";
        end if;
    end for;
    return false, "";
...
// Does N fix some primitive order-9 vector?
...
// Key for the cyclic subgroup generated by v
CyclicKey := function(v)
    S := { Eltseq((Z9!k)*v) : k in [0..8] };
    return Sprint(S);
end function;
...
StableCyclicOrd9Subgroups_old := function(Hsub)
