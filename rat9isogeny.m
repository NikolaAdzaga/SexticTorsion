...
// c.) for surviving candidates, test whether Hsub stabilizes a cyclic
//     subgroup of order 9 in (Z/9Z)^2.
//
// IMPORTANT:
// The generators in the source were intended to act on the LEFT.
// Magma uses the usual right action, e.g. for Stabilizer.
// Therefore, whenever we input a tabulated generator [a,b,c,d],
// we first make the matrix [a b; c d] and then transpose it.
...
B311 := sub< G3 |
    G3!Transpose(Matrix(Z3, 2, 2, [1,0,0,2])),
    G3!Transpose(Matrix(Z3, 2, 2, [1,1,0,1]))
>;

B312 := sub< G3 |
    G3!Transpose(Matrix(Z3, 2, 2, [2,0,0,1])),
    G3!Transpose(Matrix(Z3, 2, 2, [1,1,0,1]))
>;
...
// Compare cyclic subgroups generated by two primitive vectors
SameCyclicSubgroup := function(v, w)
    return { (Z9!k)*v : k in [0..8] } eq { (Z9!k)*w : k in [0..8] };
end function;

// Test whether Hsub stabilizes some cyclic subgroup of order 9
// (i.e. testing for rational 9-isogeny).
HasStableCyclicOrd9Subgroup := function(Hsub)
    for v in V9 do
        stable := true;
        for g in Generators(Hsub) do
...
for label in labels do
    ...
    H := sub< G9 | [ G9!Transpose(Matrix(Z9, 2, 2, g)) : g in data`gens ] >;
    ...
    for Hsub in CandidateHsubs do
        ...
        if ok9 then
            ...
            if okIso then
                printf "   Hsub stabilizes a cyclic subgroup of order 9, generated by v=%o\n", vIso;
            else
